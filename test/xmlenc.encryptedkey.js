...
describe('encrypt', function() {
  ...
  it('should encrypt and fail decrypt due to insecure algorithms', function (done) {
    ...
    xmlenc.encryptKeyInfo(plaintext, options, function(err, encryptedKeyInfo) {
      if (err) {
        // In Node.js 22+, RSA_PKCS1_PADDING is no longer supported
        if (err.message.includes('RSA_PKCS1_PADDING is no longer supported')) {
          return done(); // Test passes - correctly blocked deprecated algorithm
        }
        return done(err);
      }
